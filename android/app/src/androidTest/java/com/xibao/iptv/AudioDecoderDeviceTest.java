...
public class AudioDecoderDeviceTest {
    @Test public void packagedDecodersProducePcmForMp2AndExistingAac() throws Exception {
        assertTrue("Packaged native decoder must load on Android",FfmpegLibrary.isAvailable());
        decode("tone.mp2",MimeTypes.AUDIO_MPEG_L2);
        decode("tone.aac",MimeTypes.AUDIO_AAC);
        decode("tone.ac3",MimeTypes.AUDIO_AC3);
        decode("tone.eac3",MimeTypes.AUDIO_E_AC3);
    }
    private void decode(String file,String mime) throws Exception {
        ...
        Format format=new Format.Builder().setSampleMimeType(mime).setSampleRate(48000).setChannelCount(2).build();
