...
public class TvPlaybackTest {
    ...
    @Test public void remoteKeysRequireAuthorizationAndKeepPhoneChromeOffTelevision()throws Exception{
        try(var controller=Robolectric.buildActivity(MainActivity.class).setup()){
            ...
            a.dispatchKeyEvent(new KeyEvent(1000,1000,KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_UP,0));assertEquals(-1,get(a,"currentIndex"));
            set(a,"accessGranted",true);
            a.dispatchKeyEvent(new KeyEvent(1500,1500,KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_UP,0));assertEquals(0,get(a,"currentIndex"));assertEquals(View.GONE,((View)get(a,"navigation")).getVisibility());
            for(int i=1;i<30;i++)a.dispatchKeyEvent(new KeyEvent(1500,1500+i*10,KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_UP,i));
            assertEquals("A single held press must not skip channels",0,get(a,"currentIndex"));
            a.dispatchKeyEvent(new KeyEvent(2000,2000,KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_UP,0));assertEquals(1,get(a,"currentIndex"));
            a.dispatchKeyEvent(new KeyEvent(2500,2500,KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_DOWN,0));assertEquals(0,get(a,"currentIndex"));
